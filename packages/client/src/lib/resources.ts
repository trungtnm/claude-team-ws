...
export const epicsApi = {
  ...
  update: (projectId: string, epicId: string, data: {
    ...
    if (data.uiStatus !== undefined) body.ui_status = data.uiStatus
    if (data.gitBranches !== undefined) body.git_branches = data.gitBranches
    return api.patch<{ epic: Epic }>(`/projects/${projectId}/epics/${epicId}`, body)
  },
...
export const sessionsApi = {
  ...
  interrupt: (projectId: string, sessionId: string) =>
    api.post<{ session: AgentSession }>(`/projects/${projectId}/sessions/${sessionId}/interrupt`),
  ...
  sendMessage: (projectId: string, sessionId: string, data: {
    message: string
  ...
  delete: (projectId: string, sessionId: string) =>
    api.delete<{ status: string }>(`/projects/${projectId}/sessions/${sessionId}`),
  bulkDelete: (projectId: string) =>
    api.delete<{ deleted: number; historyDeleted: number }>(`/projects/${projectId}/sessions`),
  capabilities: (projectId: string) =>
    api.get<{ capabilities: import('@/types').SessionCapabilities }>(
      `/projects/${projectId}/sessions/capabilities`,
    ),
}
