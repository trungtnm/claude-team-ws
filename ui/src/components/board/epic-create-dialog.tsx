...
export function EpicCreateDialog({ open, onOpenChange }: EpicCreateDialogProps) {
  const { captures, updateStatus } = useCaptureStore()
...
function CaptureSelectRow({
  ...
  const user = getUserById(capture.userId)
        ...
        <div className="flex items-center gap-2 mt-1.5">
          {user && (
            <div className="flex items-center gap-1.5">
              <div
                className="flex h-4 w-4 items-center justify-center rounded-full text-[8px] font-medium text-surface-base"
