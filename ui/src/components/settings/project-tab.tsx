...
import { Camera, Save, Trash2, Copy, Check, Download, ArrowRightLeft, RefreshCw } from 'lucide-react'
...
export function ProjectTab() {
  const [name, setName] = useState('claude-team-ws')
  const [description, setDescription] = useState('Semi-Auto Epic-Driven Dev Workspace for the team. Manages Epics, spawns Claude Code agents, reviews PRs.')
  const [projectRoot, setProjectRoot] = useState('/Users/trungtran/code/claude-team-ws')
  const [maxAgents, setMaxAgents] = useState('3')
  const [askMode, setAskMode] = useState('hybrid')
  const [copied, setCopied] = useState(false)
  const [integrationStatuses, setIntegrationStatuses] = useState(integrations)
  ...
  const handleCopyId = () => {
    navigator.clipboard.writeText('proj-1').catch(() => {})
    setCopied(true)
  ...
  const handleRefreshIntegrations = () => {
    // Simulate a refresh with randomized statuses
    ...
    <div className="max-w-2xl space-y-8">
      ...
      <section className="space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <code className="rounded-[var(--radius-md)] border border-edge bg-surface-base px-3 py-1.5 text-xs font-mono text-ink-muted">proj-1</code>
            <button
              type="button"
        ...
        <div>
          ...
          <code className="rounded-[var(--radius-md)] border border-edge bg-surface-base px-3 py-1.5 text-xs font-mono text-ink-muted">claude-team-ws</code>
      ...
      <section className="space-y-4">
        ...
        <div>
          ...
          <Input value={projectRoot} onChange={(e) => setProjectRoot(e.target.value)} className="max-w-lg font-mono text-xs" />
