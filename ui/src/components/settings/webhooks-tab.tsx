...
import { Hash, MessageCircle, Plus, Send, Trash2, MoreVertical, Bot, ChevronDown, ChevronRight, ExternalLink, Copy, Check } from 'lucide-react'
...
/** Simulated metadata per webhook */
const webhookMeta: Record<string, { lastTriggered: string; healthy: boolean; failureCount: number }> = {
  'wh-1': { lastTriggered: '2 hours ago', healthy: true, failureCount: 0 },
  'wh-2': { lastTriggered: 'Never triggered', healthy: false, failureCount: 3 },
  'wh-3': { lastTriggered: '45 minutes ago', healthy: true, failureCount: 0 },
}
...
function WebhookCard({ webhook, onDelete }: { webhook: WebhookConfig; onDelete: () => void }) {
  ...
  const meta = webhookMeta[webhook.id] ?? { lastTriggered: 'Unknown', healthy: true, failureCount: 0 }

  const handleToggleEnabled = () => {
    const newState = !enabled
    setEnabled(newState)
    toast.success(`Webhook ${newState ? 'enabled' : 'disabled'}`)
  }

  const handleToggleEvent = (eventValue: string) => {
    setSelectedEvents((prev) => {
      const updated = prev.includes(eventValue)
        ? prev.filter((e) => e !== eventValue)
        : [...prev, eventValue]
      toast.success(`Event ${eventValue} ${updated.includes(eventValue) ? 'enabled' : 'disabled'}`)
      return updated
    })
  }
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="flex items-center gap-1 text-xs">
                <span className={cn(
                  ...
                  meta.healthy ? 'bg-green-400' : 'bg-red-400',
                )} />
                <span className={meta.healthy ? 'text-green-400' : 'text-red-400'}>
                  {meta.healthy ? 'Healthy' : `${meta.failureCount} failures in last 24h`}
                </span>
              </span>
            </div>
            {editingUrl ? (
            ...
            <p className="mt-0.5 text-[11px] text-ink-muted">Last triggered {meta.lastTriggered.toLowerCase()}</p>
          </div>
        </div>
...
export function WebhooksTab() {
  ...
  const handleAddWebhook = (webhook: WebhookConfig) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-ink-secondary">{webhookList.length} webhooks configured</p>
