...
type Category = KnowledgeRule['category']
type Maturity = KnowledgeRule['maturity']
...
interface RuleDialogProps {
  ...
  initialValues?: { ruleText: string; category: Category; source: string }
  mode: 'add' | 'edit'
  onSubmit: (values: { ruleText: string; category: Category }) => void
}

// Simulated AI improvements for demo
const aiImprovements: Record<string, { improved: string; category: Category; explanation: string }> = {
  default: {
...
function RuleDialog({ open, onOpenChange, initialValues, mode, onSubmit }: RuleDialogProps) {
  ...
  const [aiSuggestion, setAiSuggestion] = useState<{ improved: string; category: Category; explanation: string } | null>(null)
...
function RuleCard({ rule, onEdit }: { rule: KnowledgeRule; onEdit: () => void }) {
  const [maturity, setMaturity] = useState(rule.maturity)

  const handleApprove = () => {
    setMaturity('established')
    toast.success(`Rule "${rule.id}" approved and promoted to established`)
  }

  const handleDeprecate = () => {
    setMaturity('deprecated')
    toast.success(`Rule "${rule.id}" deprecated`)
  }
  ...
  return (
    <Card className="p-4">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <Badge className={maturityColors[maturity]}>{maturity}</Badge>
      ...
      <div className="mt-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {maturity !== 'deprecated' && (
            <Button variant="ghost" size="sm" className="text-error hover:text-error" onClick={handleDeprecate}>
              Deprecate
            </Button>
          )}
        </div>
...
export function RulesTab() {
  ...
  }, [rules, searchQuery, categoryFilter, maturityFilter])

  const candidateCount = rules.filter((r) => r.maturity === 'candidate').length

  const handleAddRule = (values: { ruleText: string; category: Category }) => {
  ...
  const handleEditRule = (values: { ruleText: string; category: Category }) => {
    if (!editingRule) return
    setRules((prev) =>
      prev.map((r) =>
        r.id === editingRule.id
          ? { ...r, ruleText: values.ruleText, category: values.category }
          : r,
      ),
    )
  ...
  const handleApproveAllCandidates = () => {
    setRules((prev) =>
      prev.map((r) => (r.maturity === 'candidate' ? { ...r, maturity: 'established' as Maturity } : r)),
    )
    toast.success(`Approved ${candidateCount} candidate rules`)
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-ink-secondary">{rules.length} knowledge rules</p>
